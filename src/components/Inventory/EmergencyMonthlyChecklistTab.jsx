...
export default function EmergencyMonthlyChecklistTab() {
  ...
  async function reloadAssets(selectFirst = true) {
    const list = await listEmergencyAssets();
    setAssets(list);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const l = await getLatestCheck("emergency_assets", selectedId);
        if (!mounted) return;
        setLatest(l);
      } catch (e) {
        console.error(e);
  ...
  async function handleSeed() {
    ...
    try {
      ...
      const list = await reloadAssets(true);
      if (list.length === 0) {
        setSeedError(
          "Seeding completed, but no assets were returned. Check Firestore rules / console."
        );
        return;
      }
      setSeedInfo(`Created/updated ${res.emergencyCount} emergency asset(s).`);
    } catch (e) {
...
<head>
<meta charset="utf-8"/>
<title>Emergency Drugs and Equipment Monthly Checklist - ${esc(
      selected.name
    )} - ${esc(mk)}</title>
<style>
...
<body>
  <h1>Emergency Drugs and Equipment Monthly Checklist - ${esc(selected.name)}</h1>
  <p class="sub">
  ...
  async function submit() {
    ...
    try {
      ...
      await createMonthlyCheck("emergency_assets", selected.id, payload);
      const l = await getLatestCheck("emergency_assets", selected.id);
      setLatest(l);
      alert("Saved monthly emergency checklist.");
    } catch (e) {
      console.error(e);
    ...
    <div className="space-y-4">
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
        <div>
          <div className="text-lg font-semibold text-slate-100">
            Emergency Drugs & Equipment - Monthly Checklist
          </div>
          <div className="text-sm text-slate-300">
            Complete and sign off the emergency trolley/equipment check.
          </div>
        </div>
      ...
      {assets.length === 0 ? (
        <div className="border border-slate-800 rounded-2xl p-4 bg-slate-950/30">
          <div className="text-slate-200">No emergency assets found.</div>
          <div className="text-xs text-slate-400 mt-1">
            Click below to create your default checklist from the seed.
          </div>
      ...
      <ChecklistManagerDialog
        ...
        onClose={async (changed) => {
          ...
          setManageDoc(null);
          if (changed) {
            const list = await reloadAssets(false);
            if (!list.find((a) => a.id === selectedId)) {
              setSelectedId(list[0]?.id || "");
            }
          }
        }}
        parentCollection="emergency_assets"
        existingIds={existingIds}
        initialDoc={manageDoc}
        title="Manage Emergency Asset Set"
        itemHasSection={true}
      />
    </div>
  );
